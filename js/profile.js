...
$(document).ready(function () {
  const profileId = window.location.href.split("?id=")[1];
  const currentUserId = getUser().id;

  if (profileId != currentUserId) {
    $("#editProfileButton").hide();
    $("#excludeProfileButton").hide();
  }
  ...
  const request = new Request({
    ...
    onSuccess: (responseData) => {
      ...
      document.getElementById("personality").innerText = getPersonalityText(
        user.personality
      );
...
$(document).on("click", "#submitEditProfileButton", function (e) {
  const request = new Request({
    ...
    data: JSON.stringify(
      Object.fromEntries(
        new FormData(document.getElementById("editProfileForm")).entries()
      )
    ),
    onSuccess: (responseData) => {
      const newUser = new User(responseData);
      const oldUser = getUser();
      newUser.tokens = oldUser.tokens;
